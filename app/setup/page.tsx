...
import { Database, CheckCircle2, HardDrive, Copy, Check, ExternalLink, ShieldCheck, Terminal, Server } from 'lucide-react';
...
-- Initial Meat Items (13 Items for Nightning POS)
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-xl flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-black text-white">คู่มือการเชื่อมต่อ Supabase & Vercel</h1>
          </div>
          <p className="text-xs text-slate-400">
            คำแนะนำขั้นตอนการตั้งค่าคลาวด์เดตาเบส Supabase และการกำหนด Environment Variables บน Vercel
          </p>
        </div>

        {/* Current Database Mode Card */}
        <div
          className={`flex items-center gap-2 px-4 py-2 rounded-2xl border text-xs font-bold ${
        ...
        >
          {isSupabaseConfigured ? (
            <>
              ...
              <span>เชื่อมต่อ Supabase สำเร็จ</span>
            </>
          ) : (
            <>
              <HardDrive className="w-4 h-4 text-amber-400" />
              <span>โหมดทดลองใช้ (LocalStorage)</span>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
          <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
            <span className="w-6 h-6 rounded-full bg-amber-500/20 flex items-center justify-center font-mono text-xs">1</span>
            <span>สร้างโปรเจกต์บน Supabase</span>
          </div>
          <p className="text-xs text-slate-400 leading-relaxed">
            1. เข้าไปที่ <a href="https://supabase.com" target="_blank" rel="noreferrer" className="text-amber-400 hover:underline flex items-center gap-1 inline-flex">supabase.com <ExternalLink className="w-3 h-3" /></a> แล้วคลิก Create New Project
            <br />
            2. ตั้งชื่อโปรเจกต์ เช่น <strong className="text-slate-200">Nightning-POS-DB</strong>
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-3">
          <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
            <span className="w-6 h-6 rounded-full bg-amber-500/20 flex items-center justify-center font-mono text-xs">2</span>
            <span>รัน SQL Script บน Supabase</span>
          </div>
          <p className="text-xs text-slate-400 leading-relaxed">
            ไปที่เมนู <strong className="text-slate-200">SQL Editor</strong> ในหน้าต่าง Supabase Dashboard คัดลอก SQL ด้านล่างนี้ไปวางและกดรันเพื่อสร้างตารางข้อมูล 13 เมนูเนื้อ
          </p>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="bg-slate-950 px-5 py-3 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-mono text-slate-300">
            ...
            <span>supabase/schema.sql (โครงสร้างตาราง & เมนูเนื้อ 13 รายการ)</span>
          </div>
          <button
        ...
        <div className="p-4 bg-slate-950 max-h-72 overflow-y-auto custom-scrollbar font-mono text-[11px] text-slate-300">
          <pre>{SQL_SCRIPT}</pre>
        </div>
      </div>

      {/* Step 3 & 4: Vercel Environment Variables */}
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl space-y-4 shadow-xl">
        <div className="flex items-center gap-2 text-amber-400 font-bold text-base">
          <Server className="w-5 h-5" />
          <span>การตั้งค่า Vercel Environment Variables</span>
        </div>

        <p className="text-xs text-slate-300 leading-relaxed">
          เมื่อคุณ Deploy โปรเจกต์นี้ขึ้น <strong className="text-amber-400">Vercel</strong> ให้เพิ่ม Environment Variables 2 ตัวนี้ในหน้าตั้งค่า Project Settings &gt; Environment Variables:
        </p>

        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3 font-mono text-xs">
          <div>
            <span className="text-slate-400 text-[10px] block mb-0.5">Key 1:</span>
            <code className="text-amber-400 font-bold">NEXT_PUBLIC_SUPABASE_URL</code>
            <p className="text-[11px] text-slate-500 font-sans mt-0.5">
              Value: URL โปรเจกต์ Supabase ของคุณ (ดูได้จาก Project Settings &gt; API ใน Supabase)
            </p>
          </div>

          <div className="pt-2 border-t border-slate-800">
            <span className="text-slate-400 text-[10px] block mb-0.5">Key 2:</span>
            <code className="text-amber-400 font-bold">NEXT_PUBLIC_SUPABASE_ANON_KEY</code>
            <p className="text-[11px] text-slate-500 font-sans mt-0.5">
              Value: Key `anon` `public` สำหรับการเข้าถึงฐานข้อมูล
            </p>
          </div>
        </div>

        <div className="p-3 bg-blue-950/60 rounded-xl border border-blue-800/60 text-xs text-blue-300 flex items-start gap-2">
          <ShieldCheck className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
          <span>
            หมายเหตุ: หากยังไม่ได้ตั้งค่า Supabase ระบบ POS จะใช้ LocalStorage เพื่อบันทึกข้อมูลการขายและจำลองการตัดสต็อกได้ 100% สมบูรณ์ทันทีโดยไม่ติดขัด!
          </span>
        </div>
      </div>
